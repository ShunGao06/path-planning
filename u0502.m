% function ReentryTime=Untitled48(delta_phi_width)
%%          调试倾侧角所用时间用的
clear;
Monte_Carlo=cell(20,1);
% 初始航向偏差为0， U0412初始航向偏差不为0
...
%     if i>=2
%         if sigma>80*pi/180%&&mark==1
%             sigma=sigma_result(i-1,1);
%             number(iii)=ii;
%             iii=iii+1;
%         elseif sigma<-80*pi/180%&&mark==1
%             sigma=sigma_result(i-1,1);
%             number(iii)=ii;
%             iii=iii+1;
%         end
...
V=V_result(i-1)
H=H_result(i-1)
lamda_oo=lamda_o/pi*180
fai_oo=fai_o/pi*180
lamdaf=lamda/pi*180
faif=fai/pi*180
lamda_finalc=dataEND(3)/pi*180
fai_finalc=dataEND(4)/pi*180

S_real=(acos(sin(fai)*sin(fai_o)+cos(fai)*cos(fai_o)*cos(lamda-lamda_o)))*Re
S_aim
CC_target=sin(pi/2-fai_final)*sin(pi/2-fai)*cos(lamda_final-lamda)+cos(pi/2-fai_final)*cos(pi/2-fai);
S_target=Re*acos(CC_target)
ReentryTime(M,2)=S_target;
